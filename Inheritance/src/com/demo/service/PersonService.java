...
public class PersonService {
	...
	public static boolean deleteData(int did)
	{
		for(int i=0; i<sarr.length; i++)
		{
			if(sarr[i]!=null)
			{
				if(sarr[i].getPid()==did)
				{
					sarr[i]=sarr[i+1];
					return true;
				}
	...
	public static boolean updateMarks(int sid, int upmark)
	{
		for(Student s1:sarr)
		{
			if(s1!=null)
			{
				if(s1.getPid()==sid)
				{
					...
					{
						((GraduateStudent)s1).setSpsub(upmark);
						return true;
					}
					else
					{
						((MasterStudent)s1).setThesismarks(upmark);
						return true;
					}
				}
			}
